    ...
    def __init__(
        ...
        if fold == 'train' or fold == 'val':
            trainvalpath = os.path.join(
                    self.path_to_images, 
                    'CheXpert-v1.0-small1/train.csv')
            ...
            if exclude_view and 'AP/PA' in self.df.columns: # 
                print("Dropping column AP PA")
                self.df.drop('AP/PA',axis=1,inplace=True)
            ...
            testpath = os.path.join(
                    self.path_to_images, 
                    'CheXpert-v1.0-small1/valid.csv')
...
class MIMICDataset(CXRDataset):
    def __init__(
        ...
        '''
        self.transform = self._transforms[fold]
        self.path_to_images = "/content/drive/MyDrive/cxr_adv/data/MIMIC_MINI/"
        self.fold = fold
        self.exclude_view= False
        ...
        if fold == 'train' or fold == 'val':
            ...
            self.df.set_index("path", inplace=True)
            if not include_lateral:
               self.df = self.df[(self.df['view'] == 'AP') | (self.df['view'] == 'PA')]
        else:
            raise ValueError("Invalid fold: {:s}".format(str(fold)))
